...
class Corte_lectordecb(models.Model):

    fecha_inicio = models.DateTimeField(auto_now_add=True)
    fecha_corte = models.DateTimeField(blank=True, null=True)
    usuario = models.CharField('Usuario', max_length=250, blank=True, null=True)

    def __str__(self):
        return unicode(self.id).encode('utf-8')

    def corte_actual(self):
        corte_lectordecb = Corte_lectordecb.objects.filter(fecha_corte=None)
        if (corte_lectordecb):
            return corte_lectordecb[0]
        else:
            return Corte_lectordecb().save()

    def hacer_corte(self, user):
        if not self.fecha_corte:
            self.fecha_corte = datetime.datetime.now()
            try:
                self.usuario = user.visible_username
            except:
                self.usuario = 'SISTEMA'
            self.save()
            return 'Corte 00'+str(self.id)+' realizado con éxito'
        return False

    def inicio_fin(self):
        fecha_inicio = timezone.localtime(self.fecha_inicio).strftime('%Y-%m-%d %X')
        if self.fecha_corte:
            fecha_corte = timezone.localtime(self.fecha_corte).strftime('%Y-%m-%d %X')
        else:
            fecha_corte = datetime.datetime.now().strftime('%Y-%m-%d %X')
        return str(parse_datetime(fecha_inicio)), str(parse_datetime(fecha_corte))
